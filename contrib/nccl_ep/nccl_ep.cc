...
ncclResult_t ncclEpCreateHandle(
    ...
    *out_handle = new ncclEpHandle();
    ...
    // Zero-padding enabled unless caller explicitly disables it.
    ...
    if (ep_group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
        ...
        // Allocate preprocessing buffers per-handle (single block allocation)
        {
            ...
            size_t sz_s2d       = align256(static_cast<size_t>(nNodes) * max_tokens * n_ranks_per_node * experts_per_rank * sizeof(int32_t));
            ...
            size_t sz_expert_offsets = align256(static_cast<size_t>(experts_per_rank) * sizeof(int64_t));
    ...
    NCCL_CHECK_RESULT(ncclEpUpdateHandle(handle, topk_idx, local_tensors, num_local_tensors, stream));
