...
struct ncclEpGroup {
    ...
    size_t ep_workspace_bytes; // Allocated size of ep_workspace
                                    ...
                                    cudaStream_t stream)
{
    ...
    // Expert-major layout: same-rank multi-expert routing duplicates tokens across
    // expert zones, so the output buffer needs headroom beyond max_recv_tokens.
    // expert_major_overalloc is in fixed-point 0.01 units (250 = 2.5x). Default: 250.
    const unsigned int em_overalloc_pct =
        (ep_group->config.expert_major_overalloc == 0 ||
         ep_group->config.expert_major_overalloc == NCCL_EP_AUTO)
        ? 250u : ep_group->config.expert_major_overalloc;
    size_t max_output_slots = (static_cast<size_t>(max_recv_tokens) * em_overalloc_pct + 99) / 100;
    size_t expert_output_token_sz = max_output_slots * hidden * sizeof(uint16_t);
    size_t expert_output_prob_sz = max_output_slots * num_local_experts * gpus_per_node * sizeof(float);
    size_t expert_input_token_sz = max_output_slots * hidden * sizeof(uint16_t);
    size_t expert_input_prob_sz = max_output_slots * num_local_experts * gpus_per_node * sizeof(float);
...
struct ncclEpHandle {
    ...
    union {
        ...
        struct {
            ...
            // Expert-major output layout (populated from ncclEpHandleConfig)
            ncclEpOutputLayout_t      dispatch_output_layout;
            size_t                    dispatch_output_per_expert_alignment;
            bool                      zero_padding_enabled;  // true = DUP Phase A zeros padding slots
            int64_t*                  expert_token_offsets;  // [experts_per_rank] internal; always written by remap
            int32_t*                  per_expert_counts_active;  // points to authoritative counts (set in UpdateHandle)
...
struct HtBlockLayout {
    ...
    static HtBlockLayout compute(ncclEpGroup_t ep_group, const ncclEpHandleConfig* config) {
        auto align256 = [](size_t s) -> size_t { return (s + 255) & ~size_t(255); };
        ...
        // Expert-major layout expands sparse_to_dense_map by experts_per_rank because the
        // remap kernel emits one slot per (token, local-expert) pair.
        const bool expert_major =
            (config != nullptr && config->dispatch_output_layout == NCCL_EP_OUTPUT_LAYOUT_EXPERT_MAJOR);
        const size_t s2d_expert_factor = expert_major ? static_cast<size_t>(experts_per_rank) : size_t(1);
        ...
        L.sz_s2d       = align256(static_cast<size_t>(nNodes) * max_tokens * n_ranks_per_node * s2d_expert_factor * sizeof(int32_t));
        ...
        // Internal offsets buffer: always written by remap kernel so kernel 2 has valid offsets
        // even when the user doesn't request them in out_meta.
        L.sz_expert_offsets = align256(static_cast<size_t>(experts_per_rank) * sizeof(int64_t));
        L.zero_region      = L.sz_routing + L.sz_r2a + L.sz_a2r + L.sz_ler + L.sz_ntfe;
        L.no_memset_region = L.sz_rank_mask + L.sz_scan_tmp + L.sz_prob + L.sz_ec_tmp + L.sz_expert_offsets;
        L.total = L.zero_region + L.sz_s2d + L.no_memset_region;
        return L;
    }
};

static size_t ht_handle_mem_size(ncclEpGroup_t ep_group, const ncclEpHandleConfig* config) {
    return HtBlockLayout::compute(ep_group, config).total;
}
...
ncclResult_t ncclEpHandleMemSize(
    ...
    if (ep_group->config.algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) {
        *size_out = ht_handle_mem_size(ep_group, config);
...
static ncclResult_t ht_init_handle(ncclEpHandle_t handle, ncclEpGroup_t ep_group, ncclNDTensor_t handle_mem, int num_topk, const ncclEpHandleConfig* config) {
    ...
    const auto L = HtBlockLayout::compute(ep_group, config);
    ...
    handle->hybridep.expert_token_offsets      = reinterpret_cast<int64_t*>(ptr + offset); offset += L.sz_expert_offsets;
...
ncclResult_t ncclEpInitHandle(
    ...
    *out_handle = new ncclEpHandle();
    ...
    return ht_init_handle(handle, ep_group, handle_mem, num_topk, config);
}
...
ncclResult_t ncclEpCreateHandle(
    ...
    NCCL_CHECK_RESULT(ncclEpInitHandle(out_handle, ep_group, config, num_topk, use_fp8, nullptr));

    // Populate feature-specific handle state (HT only) from the optional config.
    // ll_init_handle/ht_init_handle don't touch these fields, so set them here.
    if (ep_group->config.algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) {
        ncclEpHandle_t handle = *out_handle;
        ...
        handle->hybridep.zero_padding_enabled =
            !(config && config->zero_padding_mode == NCCL_EP_ZERO_PADDING_DISABLE);
    }
...
ncclResult_t ncclEpDispatch(
    ...
    if (group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
        ...
        const bool expert_major_dispatch =
            (handle->hybridep.dispatch_output_layout == NCCL_EP_OUTPUT_LAYOUT_EXPERT_MAJOR);
        ...
        // Zero-pad expert-major padding slots after dispatch, before bulk copy to recv buffer
        if (expert_major_dispatch && handle->hybridep.zero_padding_enabled) {
            const size_t alignment = std::max<size_t>(1, handle->hybridep.dispatch_output_per_expert_alignment);
            nccl_ep::hybridep::call_zero_padding(
...
ncclResult_t ncclEpHandleGetNumRecvTokens(
    ...
    if (handle->group->config.algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) {
        ...
        // Expert-major overalloc expands the buffer beyond max_recv_tokens;
        // use the same overalloc factor for the upper-bound check.
        {
            const unsigned int em_pct =
                (handle->group->config.expert_major_overalloc == 0 ||
                 handle->group->config.expert_major_overalloc == NCCL_EP_AUTO)
                ? 250u : handle->group->config.expert_major_overalloc;
            const size_t max_slots =
                (static_cast<size_t>(handle->group->max_recv_tokens) * em_pct + 99) / 100;
            assert(static_cast<size_t>(actual_recv_tokens) <= max_slots);
        }
...
// Not part of the public nccl_ep.h API.  Declared in nccl_ep_test_internal.h.
...
int ncclEpHandle_test_getMaxTokensPerRank(ncclEpHandle_t handle) {
    return handle->group->config.max_tokens_per_rank;
}

int ncclEpHandle_test_getNRanksPerNode(ncclEpHandle_t handle) {
    return handle->group->lsa_rank_count;
}
