...
static ncclEpGroup_t g_ep_group = nullptr;
...
protected:
    ...
    void SetUp() override {
        ...
        NCCL_ASSERT(ncclEpTensorCreate(g_ep_group, &topk_idx_, 2, ncclInt64,
                                       NCCL_EP_TENSOR_TAG_TOPK_IDX,
                                       d_topk_, kNumTokens, kTopK));
    }

    void TearDown() override {
        if (topk_idx_) ncclEpTensorDestroy(g_ep_group, topk_idx_);
    ...
    ncclEpHandle_t make_handle(const ncclEpHandleConfig* cfg,
        ...
        EXPECT_EQ(ncclEpCreateHandle(&h, g_ep_group, topk_idx_,
                                     cfg, local_tensors, num_local_tensors,
                                     g_stream, false),
                  ncclSuccess);
...
static bool ep_bootstrap(int argc, char* argv[], const char* uid_suffix) {
    ...
    if (comm_ret != ncclSuccess) {
        fprintf(stderr, "Rank %d: ncclCommInitRank failed (err=%d). "
                "Hint: set MPI_HOME=/usr/local/mpi so run_tests.sh uses mpirun "
                "for proper NCCL bootstrapping on EOS.\n", g_rank, comm_ret);
        return false;
    }
    cudaStreamCreate(&g_stream);

    ncclEpGroupConfig_t gcfg{};
    gcfg.version             = 1;
    gcfg.algorithm           = NCCL_EP_ALGO_HIGH_THROUGHPUT;
    gcfg.num_experts         = kNumExperts;
    gcfg.max_tokens_per_rank = kNumTokens;
    gcfg.token_size_bytes    = kHidden * sizeof(nv_bfloat16);
    gcfg.rdma_buffer_size    = NCCL_EP_AUTO;
    gcfg.num_qp_per_rank     = NCCL_EP_AUTO;
    gcfg.num_channels        = NCCL_EP_AUTO;
    ncclResult_t grp_ret = ncclEpCreateGroup(&g_ep_group, g_comm, &gcfg, g_stream, nullptr, nullptr);
...
static void ep_teardown() {
    ncclEpGroupDestroy(g_ep_group, g_stream);
