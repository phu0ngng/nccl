...
__global__ void convert_topk_to_routing_map_kernel(
    ...
    // Clear the row first so we don't OR into stale bits left by a previous iter
    // (routing_bitmap is group-scoped now, not pre-zeroed by the caller).
...
__global__ void dense_to_sparse_prob_combine_kernel(
    ...
    int64_t* __restrict__ combined_topk_idx,      // [num_tokens, topk] (optional)
    int num_tokens,
    int topk,
    int num_experts
) {
    ...
    for (int k = 0; k < topk; k++) {
        ...
        if (combined_topk_idx != nullptr) {
            combined_topk_idx[token * topk + k] = e;
        }
    }
}
...
void dense_to_sparse_prob_combine(
    ...
    dense_to_sparse_prob_combine_kernel<<<grid_size, block_size, 0, stream>>>(
        dense_prob, cached_topk_idx, combined_topk_weights, combined_topk_idx,
...
build_dispatch_param(const DispatchParams& params) {
    ...
    kp.expert_major = (params.layout == NCCL_EP_LAYOUT_EXPERT_MAJOR);
