...
struct dispatch_kernel_param_t{
  ...
  int s2d_inner_dim;               // flat: num_ranks_per_node, expert-major: num_topk
  // Layout flag (true = expert-major). Independent of pad_alignment.
  bool           expert_major;
  // Expert-major zero-padding inputs for the PAD warp.
  // PAD warp is a no-op when !expert_major or pad_alignment <= 1; pointers may be null then.
...
__forceinline__ __device__ void PAD_warp_group_device_function(
    ...
    SMEM_TYPE* smem)
{
    // No-op unless expert-major with padding requested.
    if (!expert_major || alignment <= 1 || local_buf == nullptr ||
    ...
    uint8_t* smem_bytes)
{
  ...
  if(threadIdx_x_int < INTER_NODE_GROUP::size()){
    ...
    PAD_warp_group_device_function<PAD_GROUP, TOKEN_DATA_TYPE>(
        ...
        param.experts_per_rank, param.expert_major, param.pad_alignment,
