...
echo    'set output "nccl-'$mode'-'$platform'-'$gpus'x'$nodes'.png"'
if [ "$mode" == "time" ]; then echo    'set logscale y'; fi
echo    'set title "NCCL '$coll' '$mode', '$gpus' GPUs('$platform') x '$nodes'"'
...
echo -n "plot '$path/$mode.txt'             title 'Default'     with lines lt rgb "'"#333333"'" lw 2 dt 3,"
echo -n "     '$path/Ring/LL/$mode.txt'     title 'Ring/LL'     with lines lt rgb "'"#9999FF"'" lw 1,"
echo -n "     '$path/Ring/LL128/$mode.txt'  title 'Ring/LL128'  with lines lt rgb "'"#6666FF"'" lw 1,"
echo -n "     '$path/Ring/Simple/$mode.txt' title 'Ring/Simple' with lines lt rgb "'"#0000FF"'" lw 1,"
echo -n "     '$path/Tree/LL/$mode.txt'     title 'Tree/LL'     with lines lt rgb "'"#FF9999"'" lw 1,"
echo -n "     '$path/Tree/LL128/$mode.txt'  title 'Tree/LL128'  with lines lt rgb "'"#FF6666"'" lw 1,"
echo    "     '$path/Tree/Simple/$mode.txt' title 'Tree/Simple' with lines lt rgb "'"#FF0000"'" lw 1"
) | gnuplot
