...
#ifndef USE_IR
__global__ void runDevice(ncclDevComm comm, ncclDevResourceHandle hbuf) {
  ...
  for (uint32_t round=0; round < 10; round++) {
    ...
    net.waitSignal(ncclCoopCta(), sigData0 + blockIdx.x, accum + nChunks);
...
#else
  runDevice<<<BlockPerRank, 512, 0, stream>>>(dcomm, hBuf);
