...
void runTopo(const char* xmlTopoFile, const char* platform, int nnodes) {
  ...
  for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    ...
    sprintf(path, "topo/%s/data/%d/%d/%s/%s/%s/time.txt", param.platform, ngpus, nnodes, ncclFuncStr[param.function], ncclAlgoStr[a], ncclProtoStr[p]);
    fds[i] = open(path, O_RDONLY);
  }
  sprintf(path, "topo/%s/data/%d/%d/%s/time.txt", param.platform, ngpus, nnodes, ncclFuncStr[param.function]);
  ...
  if (!param.compactMode) {
    printf("%s/%dx%d, %s\n", param.platform, nnodes, ngpus, ncclFuncStr[param.function]);
    printf("-----------+");
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    ...
    printf("%10s/%5dx%5d |", param.platform, nnodes, ngpus);
  }
