...
if [[ ${ENQUEUE_TESTS_ARGS} -eq 1 ]] ; then
  ...
  echo "Disabled Enqueue TESTS Args test\n\n"
fi
...
if [[ ${ENQUEUE_TESTS_FIFO} -eq 1 ]] ; then
  ...
  echo "Disabled Enqueue TESTS Fifo test\n\n"
fi
...
if [[ ${GRAPH_TESTS_DEFAULT} -eq 1 ]] ; then
  ...
  echo "Disabled Graph TESTS Default test\n\n"
fi
...
if [[ ${MEMLEAK_TESTS_NO_P2P} -eq 1 ]] ; then
  ...
  echo "Disabled Single-Process Mem Leak TESTS no_p2p test\n\n"
fi
...
if [[ ${MEMLEAK_TESTS_NETWORK} -eq 1 ]] ; then
  ...
  echo "Disabled Single-Process Mem Leak TESTS network test\n\n"
fi
...
if [[ ${FT_TESTS_DEFAULT} -eq 1 ]] ; then
  ...
  echo "Disabled FT TESTS Default test\n\n"
fi
...
if [[ ${FT_TESTS_NO_P2P} -eq 1 ]] ; then
  ...
  echo "Disabled FT TESTS no_p2p test\n\n"
fi
...
if [[ ${PLUGIN_TESTS_NET_TUNER} -eq 1 ]] ; then
  ...
  echo "Disabled Net/Tuner TESTS Mixed test\n\n"
fi
...
if [[ ${LOG_ALGO_RS_TESTS} -eq 1 ]] ; then
  ...
  "Disabled PAT Log Algo RS TESTS test\n\n"
fi
