...
then
  for func in all_gather_perf alltoall_perf; do
    ...
    run_command "${func}_hier_coll_graph_nvls_disable" $RUN_MODE $NGPUS "" "NCCL_MNNVL_ENABLE=0 NCCL_NET=IB NCCL_NVLS_ENABLE=0" "$NCCL_HOME/test/perf/$func" "-b 128 -e 1G -f 2 -G 1 -R 2 -x 2"
  done
fi
