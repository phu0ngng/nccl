...
if [ "$ENABLE_NVLS" == "1" ]; then
  for func in all_reduce_perf reduce_scatter_perf all_gather_perf; do
    run_command "${func}_nvls_local_registration_all_sizes" $RUN_MODE $NGPUS "-x NCCL_ALGO=NVLS" "$NCCL_HOME/test/perf/$func" "$range $opts $enable_local_register -n 1"
  done
fi

run_command "all_reduce_tree_local_registration_all_sizes" $RUN_MODE $NGPUS "-x NCCL_ALGO=Tree" "$NCCL_HOME/test/perf/all_reduce_perf" "$range $opts $enable_local_register -n 1"

for func in all_reduce_perf all_gather_perf broadcast_perf; do
  run_command "${func}_ring_local_registration_all_sizes" $RUN_MODE $NGPUS "-x NCCL_ALGO=Ring" "$NCCL_HOME/test/perf/$func" "$range $opts $enable_local_register -n 1"
done

for func in all_reduce_perf all_gather_perf broadcast_perf; do
  run_command "${func}_ring_graph_registration_all_types" $RUN_MODE $NGPUS "-x NCCL_ALGO=Ring" "$NCCL_HOME/test/perf/$func" "-b 1G -e 1G -n 5 -w 5 -d all $enable_graph_register"
done
...
then
    for func in all_reduce_perf all_gather_perf broadcast_perf; do
    run_command "${func}_ring_1rpn_graph_registration_all_types" $RUN_MODE $NGPUS "-x NCCL_ALGO=Ring -x NCCL_SHM_DISABLE=1 -x NCCL_P2P_DISABLE=1" "$NCCL_HOME/test/perf/$func" "-b $MAX -e $MAX -n 5 -w 5 -d all $enable_graph_register"
    run_command "${func}_ring_1rpn_local_registration_all_types" $RUN_MODE $NGPUS "-x NCCL_ALGO=Ring -x NCCL_SHM_DISABLE=1 -x NCCL_P2P_DISABLE=1" "$NCCL_HOME/test/perf/$func" "$range $opts $enable_local_register"
    done
fi

for func in sendrecv_perf alltoall_perf; do
  run_command "${func}_local_registration_all_sizes" $RUN_MODE $NGPUS "-x NCCL_PXN_DISABLE=1" "$NCCL_HOME/test/perf/$func" "$range $opts $enable_local_register"
done

NCCL_DEBUG_OLD=$NCCL_DEBUG
export NCCL_DEBUG=VERSION
run_command "all_reduce_output_file" $RUN_MODE $NGPUS "-x NCCL_PXN_DISABLE=1" "$NCCL_HOME/test/perf/all_reduce_perf" "-b8 -e8 -w0 -n1 -J test_out.json"
...
then
  ...
  run_command "ft_test_skip_init_${SKIP_FT_INIT}_skip_finalize_${SKIP_FT_FINALIZE}" $RUN_MODE $NGPUS "-x NCCL_SOCKET_RETRY_SLEEP_MSEC=1 " "$NCCL_HOME/test/perf/all_reduce_perf" "$range $opts $enable_ft"
...
for func in all_reduce_perf alltoall_perf; do
  run_command "${func}_nic_fusion_phb"      $RUN_MODE $NGPUS "-x NCCL_NET_MERGE_LEVEL=PHB " "$NCCL_HOME/test/perf/$func" "-b 8 -e 128M -f2 $opts -n 1"
  run_command "${func}_nic_fusion_phb_1ppn" $RUN_MODE 1      "-x NCCL_NET_MERGE_LEVEL=PHB " "$NCCL_HOME/test/perf/$func" "-b 8 -e 128M -f2 $opts -t $NGPUS -n 1"
done
