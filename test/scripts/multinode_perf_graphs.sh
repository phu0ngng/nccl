...
$salloc_cmd mpirun $prefix $mpi_hosts --bind-to none -x NCCL_DEBUG -x NCCL_LAUNCH_MODE -np $nproc -npernode $npn test/perf/${op}_perf -t $nthread -g $ngpus -b 40000 -e 1960000 -i 40000 -w 5 -n 20 2>&1 | tee $result.out
$salloc_cmd mpirun $prefix $mpi_hosts --bind-to none -x NCCL_DEBUG -x NCCL_LAUNCH_MODE -np $nproc -npernode $npn test/perf/${op}_perf -t $nthread -g $ngpus -b 2000000 -e 38000000 -i 2000000 -w 2 -n 5 2>&1 | tee -a $result.out
$salloc_cmd mpirun $prefix $mpi_hosts --bind-to none -x NCCL_DEBUG -x NCCL_LAUNCH_MODE -np $nproc -npernode $npn test/perf/${op}_perf -t $nthread -g $ngpus -b 40000000 -e 400000000 -i 40000000 -c 0 -w 1 -n 2 2>&1 | tee -a $result.out
