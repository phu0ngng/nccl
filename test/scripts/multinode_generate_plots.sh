...
if [ "$mode" == "bw" ]; then 
  ...
  plot_ngpu_loop reduce 10 $gpumodel $@
  plot_ngpu_loop broadcast 5 $gpumodel $@
  plot_ngpu_loop all_reduce 9 $gpumodel $@
  plot_ngpu_loop all_gather 8 $gpumodel $@
  plot_ngpu_loop reduce_scatter 9 $gpumodel $@
else
  echo "Invalid mode. Valid modes are bw or lat."
fi
echo "done."
