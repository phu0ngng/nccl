...
mpiexec --allow-run-as-root -np 8 cntk configFile=/data/networks/ResNet50/ResNet50.cntk meanFile=/data/images/ImageNet1K_mean.xml ConfigDir=/data/networks/ResNet50 trainMapFile=/data/map/fake_trainMap.txt OutputPath=/tmp/cntk-$(date +"%Y%m%d%I%M%S") epochSize=51200 batchSize=512 parallelTrain=true 2>&1 | \
