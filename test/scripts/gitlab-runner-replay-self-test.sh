...
[ $? -ne 0 ] && let failure_count=$failure_count+1 && failure_names+=("Generate all_reduce trace file:\n mpirun $MPI_PARAMS -x NCCL_DEBUG_SUBSYS=CALL -x NCCL_DEBUG=TRACE -x NCCL_DEBUG_FILE=trace_all_reduce.%h.%p ./build/test/perf/all_reduce_perf -w5 -n5 -b8 -e10M -f2 -d all")
...
[ $? -ne 0 ] && let failure_count=$failure_count+1 && failure_names+=("Generate alltoall trace file:\n mpirun $MPI_PARAMS -x NCCL_DEBUG_SUBSYS=CALL -x NCCL_DEBUG=TRACE -x NCCL_DEBUG_FILE=trace_alltoall.%h.%p ./build/test/perf/alltoall_perf -w5 -n5 -b1K -e10M -f2")
