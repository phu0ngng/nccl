...
# This script just logs into the selene-login node via the jumpbox and invokes each step
...
/usr/bin/ssh selene-ci mkdir -p $script_dir
/usr/bin/scp ${1} selene-ci:${1}
/usr/bin/ssh selene-ci "PATH=~/bin:$PATH && ${1}"