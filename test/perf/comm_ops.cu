 ...
 * initialization, splitting, and shrinking. Unlike other tests in test/perf/,
...
void parse_args(int argc, char **argv) {
  ...
  static struct option long_options[] = {
    ...
    {"warmup-exclude", required_argument, 0, 'w'},
  ...
  while ((opt = getopt_long(argc, argv, "aAb:e:F:hi:k:r:R:s:Svw:W:", long_options, &option_index)) != -1) {
    switch (opt) {
      ...
      case 'v':
        ...
        break;
      case 'w':
        args.warmup_exclude = atoi(optarg);
        if (args.warmup_exclude < 0) {
          fprintf(stderr, "Error: Warmup exclude must be greater than or equal to 0\n");
          ABORT_EXIT;
        }
        break;
      case 'W':
...
void gather_timing(timer_record_t* timing) {
  ...
  if (timing->rank == 0) {
    ...
    for (int jj=0; jj<timing->count; jj++) {
      int id = timing->id[jj];
      if (timing->rounds_by_type[id] >= timing->warmup_exclude) {
 ...
 */
timer_record_t* timing_allocate(MPI_Comm comm, int flush, int warmup) {
  timer_record_t* timing = (timer_record_t*)malloc(sizeof(timer_record_t));
  timing->flush_interval = flush;
  timing->warmup_exclude = warmup;
...
void timing_print_summary_grouped(timer_record_t** timings, int num_timings) {
  ...
  for (int i=0; i<timer_enum_count; i++) {
    ...
    for (int k=0; k<num_timings; k++) {
      ...
      int64_t rounds_counted = timing->rounds_by_type[i] - timing->warmup_exclude;
...
int main(int argc, char **argv) {
  ...
  timing_prempi = timing_allocate(MPI_COMM_NULL, 16, 0);
...
void print_help() {
  ...
  if (rank == 0) {
    ...
    printf("    -i|--iterations <iterations>: number of iterations to run (Default: %d).\n", args.iterations);
    ...
    printf("    -w|--warmup-exclude <exclude>: exclude first <exclude> rounds from summaries. (Default to %d).\n", args.warmup_exclude);
    printf("Examples:\n");
