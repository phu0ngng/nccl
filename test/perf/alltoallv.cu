 ...
 *    - ALLTOALLV_SPREAD: Controls the distance-weighted spread factor.
 ...
 *   Set ALLTOALLV_PRINT_SUMMARY=1 to print detailed per-rank statistics
 ...
 *   ALLTOALLV_SPREAD=0.0 alltoallv_perf -b 1M -e 32M -f 2
...
static void AlltoAllvParseEnv() {
  ...
  const char* print_summary_env = getenv("ALLTOALLV_PRINT_SUMMARY");
  if (print_summary_env) alltoallv_print_summary = atoi(print_summary_env) != 0;
  const char* spread_env = getenv("ALLTOALLV_SPREAD");
  if (spread_env) {
    errno = 0;
    char* end = NULL;
  ...
  const char* scale_env = getenv("ALLTOALLV_MATRIX_SCALE");
  if (scale_env) {
    errno = 0;
    char* end = NULL;
...
testResult_t AlltoAllvInitData(struct threadArgs* args, ncclDataType_t type, ncclRedOp_t op, int root, int rep, int in_place) {
  ...
  for (int id = 0; id < args->commNum; ++id) {
    for (int i=0; i<args->nGpus; i++) {
      ...
      size_t total_bytes = alltoallv_use_generated_pattern ? args->nbytes[id][i] * nranks : 0;
      ...
      for (int src = 0; src < nranks; src++) {
        ...
        TESTCHECK(InitData((char*)args->expected[id][i] + recvoffset, recvcount, src_offset, type, ncclSum, 33*rep + src, 1, 0));
...
static void AlltoAllvPrintRankBandwidths(int nranks, size_t total_bytes, double sec) {
  if (alltoallv_use_generated_pattern) {
    printf("# ALLTOALLV_PRINT_SUMMARY: mode=generated nranks=%d total_bytes=%zu spread=%.3f time_usec=%.2f\n",
           nranks, total_bytes, distance_weighted_spread, sec * 1.0E6);
  } else {
    printf("# ALLTOALLV_PRINT_SUMMARY: mode=matrix nranks=%d matrix_dim=%d scale=%.3f time_usec=%.2f\n",
