...
void ReduceScattervInitRecvResult(struct threadArgs_t* args, ncclDataType_t type, ncclRedOp_t op, int root, int in_place, int is_first) {
...
void ReduceScattervRunColl(void* sendbuff, void* recvbuff, size_t count, ncclDataType_t type, ncclRedOp_t op, int root, ncclComm_t comm, cudaStream_t stream) {
  NCCLCHECK(ncclReduceScatterv(sendbuff, recvbuff, count, type, op, comm, stream));
}

struct ncclColl_t reduceScatterv = {
  "ReduceScatterv",
