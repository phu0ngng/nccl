...
testResult_t ReduceInitRecvResult(struct threadArgs* args, ncclDataType_t type, ncclRedOp_t op, int root, int in_place, int is_first) {
  ...
  for (int i=0; i<args->nGpus; i++) {
    ...
    if (is_first && i == 0) {
      ...
      TESTCHECK(Accumulate(args->procShared, data, count, type, op));
    }
    ...
    if (args->expectedBytes) {
      ...
      if (root_proc == args->proc) {
        for (int i=0; i<args->nProcs; i++) {
          ...
          TESTCHECK(Accumulate(args->procShared, remote, count, type, op));
          CUDACHECK(cudaDeviceSynchronize());
        }
      } else {
