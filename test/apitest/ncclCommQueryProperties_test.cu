...
class ncclCommQueryProperties_test : public ncclCommon_test<char> {
  protected:
    ...
    void SetUp() override {
      // Unlike ncclCommon_test, this class re-initializes the communicators between every test.
      // This is required because some tests expect devCommCreate to fail, which puts the comms in a bad state.
...
TEST_F(ncclCommQueryProperties_test, test_device_api_support) {
  ncclCommProperties_t props = NCCL_COMM_PROPERTIES_INITIALIZER;
  ASSERT_EQ(ncclSuccess, ncclCommQueryProperties(comms[0], &props));
  ncclDevCommRequirements reqs = NCCL_DEV_COMM_REQUIREMENTS_INITIALIZER;

  bool expectSuccess = props.deviceApiSupport;
  ASSERT_EQ(ncclSuccess, ncclGroupStart());
  for (int i = 0; i < nVis; i++) {
    if (expectSuccess) {
      ASSERT_EQ(ncclSuccess, ncclDevCommCreate(comms[i], &reqs, &devComms[i]));
    } else {
      ASSERT_NE(ncclSuccess, ncclDevCommCreate(comms[i], &reqs, &devComms[i]));
    }
  }
  if (expectSuccess) {
