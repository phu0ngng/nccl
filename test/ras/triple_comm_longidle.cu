...
// each of the existing two communicators.  After that it sleeps for 60
// seconds.
//
// The test is identical to triple_comm_idle, apart from the sleep at the
// end, which is there to give RAS an opportunity to gracefully shut down
// the no longer needed RAS connections.  To test that aspect, 6+ ranks
// total are needed (with 4 ranks, there won't be any no longer needed
// connections).
//
#include <cassert>
#include <cstdio>
