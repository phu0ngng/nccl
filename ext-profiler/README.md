...
`ncclProfileProxyOp`, `ncclProfileProxyStep`, `ncclProfileProxyCtrl` and `ncclProfileKernelCh`.
...
thread. Thus, the proxy progress thread infrastructure is shared between the network and the kernel. If
...
delayed, a similar loss of accuracy can be encountered. Keeps this in mind when using kernel events.
