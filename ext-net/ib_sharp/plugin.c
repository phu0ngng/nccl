...
ncclResult_t ncclIbPtrSupport(int dev, int* supportedTypes) {
  *supportedTypes = NCCL_PTR_HOST;
  ...
  if (ncclIbGdrSupport(dev) != ncclSuccess) {
    INFO(NCCL_NET,"NET/IB : GPU Direct RDMA Disabled for GPU %d[%d] / HCA %d '%s' (no module or not supported by GPU)", cudaDev, dev, ncclIbDevs[dev].devName);
    return ncclSuccess;
  }
...
ncclResult_t ncclSharpRegMr(void* comm, void* data, int size, int type, void** mhandle) {
  return ncclSuccess;
}

ncclResult_t ncclSharpDeregMr(void* comm, void* mhandle) {
  return ncclSuccess;
}
