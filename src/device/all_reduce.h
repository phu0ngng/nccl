...
namespace {
  ...
  __device__ __forceinline__ void treeTop(ncclWorkElem *args, int tid, int nthreads, int* down, ssize_t loopSize, ssize_t chunkSize, ssize_t size, int connIndex) {
    ...
    for (ssize_t  elemOffset = 0; elemOffset < size; elemOffset += loopSize) {
      ssize_t chunkOffset = elemOffset + connIndex * int(chunkSize);
      ssize_t offset = gridOffset + chunkOffset;
      int nelem = min(chunkSize, size-chunkOffset);
      prims.directRecvReduceCopySend(offset, offset, nelem, /*doPost=*/true);
    }
  }

  template<typename T, typename RedOp, typename Proto, int DownArity, int UpArity>
  __device__ __forceinline__ void treeReduce(ncclWorkElem *args, int tid, int nthreads, int* down, int *up, ssize_t loopSize, ssize_t chunkSize, ssize_t size, int connIndex) {
    const ssize_t gridOffset = args->workOffset;
    Primitives<T, RedOp, FanAsymmetric<DownArity, UpArity>, /*Direct=*/1, Proto, 0>
      prims(tid, nthreads, down, up, args->sendbuff, args->recvbuff, args->redOpArg, 2*connIndex*Proto::MaxGroupWidth, connIndex, connIndex);
    if (down[0] == -1) {
      for (ssize_t elemOffset = 0; elemOffset < size; elemOffset += loopSize) {
        ssize_t chunkOffset = elemOffset + connIndex * int(chunkSize);
        ssize_t offset = gridOffset + chunkOffset;
  ...
  template<typename T, typename RedOp, typename Proto, int UpArity, int DownArity>
  __device__ __forceinline__ void treeBroadcast(ncclWorkElem *args, int tid, int nthreads, int* up, int* down, ssize_t loopSize, ssize_t chunkSize, ssize_t size, int connIndex) {
    const ssize_t gridOffset = args->workOffset;
    Primitives<T, RedOp, FanAsymmetric<UpArity, DownArity>, /*Direct=*/1, Proto, 0>
      prims(tid, nthreads, up, down, args->sendbuff, args->recvbuff, args->redOpArg, (2*connIndex+1)*Proto::MaxGroupWidth, connIndex, connIndex);
    if (down[0] == -1) {
      for (ssize_t elemOffset = 0; elemOffset < size; elemOffset += loopSize) {
        ssize_t chunkOffset = elemOffset + connIndex * int(chunkSize);
        ssize_t offset = gridOffset + elemOffset + connIndex * int(chunkSize);
        int nelem = min(chunkSize, size-chunkOffset);
        prims.directRecv(offset, nelem);
      }
    } else {
  ...
  __device__ __forceinline__ void runTreeSplit(ncclWorkElem *args) {
    ...
    const ssize_t loopSize = args->chunkCount;
    const ssize_t chunkSize = loopSize >> 1; // Each sub-tree processes half the chunk
    const ssize_t size = args->workCount;

    int nthreadsSplit[] = { 128, 96, 128, 96 };
    // Use more threads for the tree with more arity
    nthreadsSplit[tree->downTree*2] += 96;
    nthreadsSplit[tree->downTree*2+1] += 96;
    for (int t=0; t<2; t++) {
      if (tree->up[t] == -1) {
        nthreadsSplit[t*2] += nthreadsSplit[t*2+1];
        nthreadsSplit[t*2+1] = 0;
      }
    }

    int* nthreadsPtr = nthreadsSplit;
    for (int treeIndex=0; treeIndex<2; treeIndex++) {
      int up = tree->up[treeIndex];
      int* down = tree->down[treeIndex];
      if (tid < *nthreadsPtr) {
        if (up == -1) {
          treeTop<T, RedOp, Proto, NCCL_MAX_TREE_ARITY>(args, tid, *nthreadsPtr, down, loopSize, chunkSize, size, treeIndex);
        } else {
          if (treeIndex == tree->downTree) {
            treeReduce<T, RedOp, Proto, NCCL_MAX_TREE_ARITY, 1>(args, tid, *nthreadsPtr, down, &up, loopSize, chunkSize, size, treeIndex);
          } else {
            treeReduce<T, RedOp, Proto, 1, 1>(args, tid, *nthreadsPtr, down, &up, loopSize, chunkSize, size, treeIndex);
          }
        }
        break;
      }
      ...
      if (tid < *nthreadsPtr) {
        if (treeIndex == tree->downTree) {
          treeBroadcast<T, RedOp, Proto, 1, NCCL_MAX_TREE_ARITY>(args, tid, *nthreadsPtr, &up, down, loopSize, chunkSize, size, treeIndex);
        } else {
          treeBroadcast<T, RedOp, Proto, 1, 1>(args, tid, *nthreadsPtr, &up, down, loopSize, chunkSize, size, treeIndex);
        }
        break;
      }
      tid -= *nthreadsPtr;
...
struct RunWorkElement<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_COLLNET_CHAIN, NCCL_PROTO_SIMPLE> {
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    ...
    const int bid = args->bid;
    const int nChannels = args->nChannels;
    ncclChain *chain = &ncclShmem.channel.collnetChain;
    ssize_t chunkSize = args->chunkCount;
