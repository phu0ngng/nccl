...
template<int EltSize, int Unroll, int NumDsts1, int MinDsts2, int MaxDsts2, int MaxDstAddrs,
         typename DstPtrFn, typename IntBytes>
__device__ __forceinline__ void copyOneToMany_SrcAligned16(
    int nWarps, int warp, int lane,
    int nDsts2, uintptr_t dstAddrs[/*MaxDstAddrs*/], DstPtrFn const &dstPtrFn, bool dsts2Aligned,
  ...
  while (Unroll*WARP_SIZE*16 <= bytesAhead) {
    ...
    if (dsts2Aligned) {
      ...
      for (int d=NumDsts1; d < MaxDstAddrs; d++) {
        if (NumDsts1+MinDsts2 <= d && NumDsts1+nDsts2 <= d) break;
        #pragma unroll Unroll
        for (int u=0; u < Unroll; u++) {
      ...
      for (int d=MaxDstAddrs; d < MaxDsts && d < NumDsts1+nDsts2; d++) {
        uintptr_t dstAddr = cvta_to_global(dstPtrFn(d)) + bytesBehind;
        #pragma unroll Unroll
        for (int u=0; u < Unroll; u++) {
    ...
    if (NumDsts1!=0 || (MaxDsts2!=0 && !dsts2Aligned)) {
      ...
      for (int d=0; d < NumDsts1+(dsts2Aligned ? 0 : nDsts2); d++) {
        uintptr_t dstAddr = (MaxDsts==1) ? dstAddrs[0] : cvta_to_global(dstPtrFn(d)) + bytesBehind;
...
template<int EltSize, int Unroll, bool Diverge, int MinDsts, int MaxDsts, int MaxDstAddrs,
         typename DstPtrFn, typename IntBytes>
__device__ __forceinline__ void copyOneToMany_EltWise(
    int nWarps, int warp, int lane, int nDsts, uintptr_t dstAddrs[/*MaxDstAddrs*/], DstPtrFn const &dstPtrFn,
    uintptr_t& srcAddr, IntBytes& bytesBehind, IntBytes& bytesAhead, IntBytes& itersAhead
  ) {
  static_assert(IntBytes(-1)>>1 == IntBytes(-1), "IntBytes must be signed");
...
template<int EltSize, int NumDsts1, int MinDsts2_, int MaxDsts2,
         bool Dsts2Aligned, bool SrcAligned,
         typename IntBytes, typename DstPtrFn>
__device__ __forceinline__ void copyOneToMany(
    int nWarps, int warp, int lane, int nDsts2,
    DstPtrFn const &dstPtrFn, void* srcPtr,
    IntBytes bytesAhead, uint32_t& scratchAddr
  ) {
  static_assert(IntBytes(-1)>>1 == IntBytes(-1), "IntBytes must be signed");
  ...
  #pragma unroll
  for (int d=0; d < MaxDstAddrs; d++) {
    if (d < MinDsts || d < NumDsts1+nDsts2) dstAddrs[d] = cvta_to_global(dstPtrFn(d));
  }
      ...
      if (lane*EltSize < delta) {
        ...
        for (int d=0; d < MaxDsts; d++) {
          if (d < MinDsts || d < NumDsts1+nDsts2) {
            uintptr_t dstAddr = (d < MaxDstAddrs) ? dstAddrs[d] : cvta_to_global(dstPtrFn(d));
  ...
  for (int d=0; d < NumDsts1; d++) {
    uintptr_t dstAddr = (d < MaxDstAddrs) ? dstAddrs[d] : cvta_to_global(dstPtrFn(d)) + bytesBehind;
  ...
  if (dsts1Aligned && dsts2Aligned) {
    copyOneToMany_SrcAligned16
      <EltSize, Unroll, /*NumDsts1=*/0, /*MinDsts2=*/MinDsts, /*MaxDsts2=*/MaxDsts, MaxDstAddrs>
        (nWarps, warp, lane, NumDsts1+nDsts2, dstAddrs, dstPtrFn, /*dsts2Aligned=*/true,
    ...
    copyOneToMany_SrcAligned16
      <EltSize, Unroll, /*NumDsts1=*/NumDsts1, /*MinDsts2=*/MinDsts2, /*MaxDsts2=*/MaxDsts2, MaxDstAddrs>
        (nWarps, warp, lane, nDsts2, dstAddrs, dstPtrFn, dsts2Aligned,
         srcAddr, bytesBehind, bytesAhead, itersAhead, scratchAddr);
  } else {
    copyOneToMany_EltWise
      <EltSize, Unroll*16/EltSize, /*Diverge=*/false, MinDsts, MaxDsts, MaxDstAddrs>
        (nWarps, warp, lane, NumDsts1+nDsts2, dstAddrs, dstPtrFn,
  ...
  if (dsts1Aligned && dsts2Aligned) {
    ...
    for (int d=0; d < MaxDsts; d++) {
      if (d < MinDsts || d < NumDsts1+nDsts2) {
        allDstAddrs[d] = (d < MaxDstAddrs) ? dstAddrs[d] : cvta_to_global(dstPtrFn(d)) + bytesBehind;
      }
    }
    copyOneToMany_EltWise</*EltSize=*/16, /*Unroll=*/1, /*Diverge=*/true, MinDsts, MaxDsts, MaxDsts>
      (nWarps, warp, lane, NumDsts1+nDsts2, allDstAddrs, dstPtrFn, srcAddr,
    ...
    copyOneToMany_EltWise<EltSize, /*Unroll=*/4, /*Diverge=*/true, MinDsts, MaxDsts, MaxDstAddrs>
      (nWarps, warp, lane, NumDsts1+nDsts2, dstAddrs, dstPtrFn, srcAddr,
       bytesBehind, bytesAhead, /*ignored*/itersAhead);
  }
}
...
__device__ __forceinline__ void reduceCopy(
    ...
    uint64_t redArg, uint64_t *preOpArgs, bool postOp,
    int nSrcs, SrcPtrFn const &srcPtrFn, int nDsts, DstPtrFn const &dstPtrFn,
    IntBytes nElts, uint32_t& scratch) {
    ...
    if (MultimemSrcs+MultimemDsts == 0 && MinSrcs==1 && MaxSrcs==1 &&
      ...
      if (MinDsts > 0 || nDsts > 0) {
        copyOneToMany<sizeof(T), /*NumDsts1=*/1, /*MinDsts2=*/0, /*MaxDsts2=*/MaxDsts-1,
          ...
          (nWarps, warp, lane, /*nDsts2=*/nDsts-1, dstPtrFn, srcPtrFn(0),
          IntBytes(nElts*sizeof(T)), scratch);
      }
    } else {
      reduceCopyFull<Unroll, RedFn, T,
                 MultimemSrcs, MinSrcs, MaxSrcs,
                 MultimemDsts, MinDsts, MaxDsts, PreOpSrcs, IntBytes>
                (thread, nThreads, redArg, preOpArgs, postOp,
                nSrcs, srcPtrFn,
                nDsts, dstPtrFn, nElts);
    }
  }
}
...
__device__ __forceinline__ void reduceCopy(
  ...
  reduceCopy<Unroll, RedFn, T,
    ...
    (thread, nThreads, redArg, preOpArgs, postOp,
     ...
     nDsts, [=]__device__(int i) { return dstPtrs[i]; }, nElts, scratch);
}
