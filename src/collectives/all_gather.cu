...
ncclResult_t ncclAllGatherFunc(const void* sendbuff, void* recvbuff, size_t count,
  ...
  TRACE("opCount %lx sendbuff %p recvbuff %p count %zi/%zi datatype %d op %d comm %p [nranks=%d] stream %p", comm->opCount, sendbuff, recvbuff, count, nbytes, datatype, op, comm, comm->nRanks, stream);
  if (comm->nRanks == 1) {
