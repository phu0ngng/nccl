...
__device__ void ncclSendRecvKernel(struct CollectiveArgs* args) {
  ...
  //if(tid==0) printf("rank %d[ch%d] sendrecv kernel %ld send %ld recv peer %d\n",comm->rank,blockIdx.x,sendSize,recvSize,args->root);
  
  ncclPrimitives<UNROLL, SENDRECV_CHUNKSTEPS/SENDRECV_SLICESTEPS, SENDRECV_SLICESTEPS, T, 1, 1, FUNC>
    prims(tid, args->nThreads, &peer, &peer, NULL, stepSize, channel, comm, args->opCount);

  for (ssize_t gridOffset = 0; gridOffset < sendSize; gridOffset += loopSize) {
  ...
  for (ssize_t gridOffset = 0; gridOffset < recvSize; gridOffset += loopSize) {
