...
class Primitives<
  ...
  __device__ __forceinline__ void loadRecvConn(ncclPeer *peer) {
    if (flags & (RoleWaitRecv|RolePostRecv)) {
      // TODO: This is ugly. We should be accepting connIndex from the constructor, not
      // inferring it from our send/recv max bounds.
      // For oneshot: groups 0,2 use conn 0, groups 4,6 use conn 1
      const int connIndex = (MaxSend == NCCL_MAX_DIRECT_ARITY || MaxRecv == NCCL_MAX_DIRECT_ARITY) ? group/4 : 0;
 ...
 public:
  __device__ Primitives(
    ...
    while (nrecv < MaxRecv && recvPeers[nrecv] != -1) {
      //if (tid==0) printf("%d recvs %d\n", ncclShmem.comm.rank, recvPeers[nrecv]);
      nrecv++;
    }
    while (nsend < MaxSend && sendPeers[nsend] != -1) {
      //if (tid==0) printf("%d sends %d\n", ncclShmem.comm.rank, sendPeers[nsend]);
      nsend++;
    }
