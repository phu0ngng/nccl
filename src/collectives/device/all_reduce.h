...
__device__ void ncclAllReduceTreeLLKernel(struct CollectiveArgs* args) {
  ...
  // to 3 dests. Use more threads on the reduce side.
  int nthreadsSplit = nthreads/2;
  if (nthreadsSplit >= 128) nthreadsSplit += nthreads/4;
  ...
  if (tree->up == -1) {
    ...
    ncclLL128Primitives<T, FUNC, NCCL_MAX_TREE_ARITY, NCCL_MAX_TREE_ARITY> LLprims(tid, nthreads, tree->down, tree->down, channel, comm, args->opCount);
    for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
      ssize_t offset = gridOffset + bid*chunkSize;
      int nelem = min(chunkSize, size-offset);
      LLprims.recvReduceCopySend(thisInput+offset, thisOutput+offset, nelem);
