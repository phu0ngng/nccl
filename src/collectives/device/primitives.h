...
class ncclPrimitives {
 private:
  ...
  template <int DIRECTRECV, int DIRECTSEND, int RECV, int SEND, int SRC, int DST, int OFFSET>
  inline __device__ void
  GenericOp(const T* srcPtr, T* dstPtr, int nelem, ssize_t directOffset, int peerIn) {
    int offset = 0;
    int sliceSize = stepSize*SLICESTEPS;
    int dataSize = max(DIVUP(nelem, 16*SLICESPERCHUNK)*16, sliceSize/32);
    int realNsend = OFFSET ? 1 : nsend;
    int realNrecv = OFFSET ? 1 : nrecv;
    ...
    for (int slice=0; slice<SLICESPERCHUNK; ++slice) {
      ...
      if (tid < nworkers) {
        ...
        if (RECV && (role & ROLE_WAIT_RECV) && (!OFFSET || peer == peerIn)) waitRecv<SRC, DIRECTRECV>(directOffset+offset);
        if (DST && (role & ROLE_DST)) dsts[0] = dstPtr+offset;
        if (SEND && (role & ROLE_WAIT_SEND) && (!OFFSET || peer == peerIn)) waitSend<DST, DIRECTSEND>(directOffset+offset, realSize*sizeof(T));
        if (realSize > 0) {
          subBarrier();
          if (DIRECTRECV && srcs[0] == dsts[0]) {
            ...
            if (SEND) {
              ...
              ReduceOrCopyMulti<UNROLL, FUNC, T, 1, 1, 1, (1-SEND)+NSEND>(tid, nworkers, 1, srcs, realNsend, dsts+1, realSize);
            }
          } else {
            ReduceOrCopyMulti<UNROLL, FUNC, T, RECV+SRC, RECV*NRECV+SRC, SEND+DST, SEND*NSEND+DST>(tid, nworkers, RECV*realNrecv+SRC, srcs, SEND*realNsend+DST, dsts, realSize);
          }
        }
      }
      barrier();
      ...
      __syncwarp();
      if (SEND && (role & ROLE_POST_SEND) && (!OFFSET || peer == peerIn)) postSend();
      if (RECV && (role & ROLE_POST_RECV) && (!OFFSET || peer == peerIn)) postRecv();
      offset += realSize;
    }
  }
  ...
  __device__ __forceinline__ void loadRecvConn(struct ncclChannel* channel, T* directBuff) {
    if (role & (ROLE_WAIT_RECV|ROLE_POST_RECV)) {
      ...
      conn = &channel->devPeers[peer].recv[group/4].conn;
      step = conn->step;
  ...
  __device__ __forceinline__ void loadSendConn(struct ncclChannel* channel) {
    if (role & (ROLE_WAIT_SEND|ROLE_POST_SEND)) {
      // Groups 0,2 use conn 0, groups 4,6 use conn 1
      conn = &channel->devPeers[peer].send[group/4].conn;
      step = conn->step;
      step = ROUNDUP(step, SLICESPERCHUNK*SLICESTEPS);
 ...
 public:
  ...
  send(const T* src, int nelem) {
    GenericOp<0, 0, 0, 1, 1, 0, 0>(src, NULL, nelem, 0, -1);
  }
  __device__ __forceinline__ void
  directSend(const T* src, ssize_t directOffset, int nelem) {
    GenericOp<0, 1, 0, 1, 1, 0, 0>(src, NULL, nelem, directOffset, -1);
  }
  __device__ __forceinline__ void
  ...
  recv(T* dst, int nelem) {
    GenericOp<0, 0, 1, 0, 0, 1, 0>(NULL, dst, nelem, 0, -1);
  }
  __device__ __forceinline__ void
  directRecv(T* dst, ssize_t directOffset, int nelem) {
    GenericOp<1, 0, 1, 0, 0, 1, 0>(NULL, dst, nelem, directOffset, -1);
  }
  __device__ __forceinline__ void
  ...
  copySend(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 0, 1, 1, 1, 0>(src, dst, nelem, 0, -1);
  }
  __device__ __forceinline__ void
  directCopySend(const T* src, T* dst, ssize_t directOffset, int nelem) {
    GenericOp<0, 1, 0, 1, 1, 1, 0>(src, dst, nelem, directOffset, -1);
  }

  __device__ __forceinline__ void
  recvCopySend(T* dst, int nelem) {
    GenericOp<0, 0, 1, 1, 0, 1, 0>(NULL, dst, nelem, 0, -1);
  }
  __device__ __forceinline__ void
  directRecvCopySend(T* dst, ssize_t directOffset, int nelem) {
    GenericOp<1, 1, 1, 1, 0, 1, 0>(NULL, dst, nelem, directOffset, -1);
  }

  __device__ __forceinline__ void
  recvReduceCopy(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 1, 0, 1, 1, 0>(src, dst, nelem, 0, -1);
  }

  __device__ __forceinline__ void
  recvReduceSend(const T* src, int nelem) {
    GenericOp<0, 0, 1, 1, 1, 0, 0>(src, NULL, nelem, 0, -1);
  }

  __device__ __forceinline__ void
  recvReduceCopySend(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 1, 1, 1, 1, 0>(src, dst, nelem, 0, -1);
  }
  __device__ __forceinline__ void
