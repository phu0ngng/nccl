...
ncclResult_t ncclReduceFunc(const void* sendbuff, void* recvbuff, const size_t count,
  ...
  INFO(COLL,"Reduce: opCount %lx sendbuff %p recvbuff %p count %zi size %zi datatype %d op %d root %d comm %p [nranks=%d] stream %p", comm->opCount, sendbuff, recvbuff, count, nbytes, datatype, op, root, comm, comm->nRanks, stream);
  if (comm->nRanks == 1) {
