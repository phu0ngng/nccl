...
ncclResult_t ncclReduceScatterFunc(const void* sendbuff, void* recvbuff, size_t count,
  ...
  INFO(COLL,"opCount %lx sendbuff %p recvbuff %p count %zi/%zi datatype %d op %d comm %p [nranks=%d] stream %p", comm->opCount, sendbuff, recvbuff, count, nbytes, datatype, op, comm, comm->nRanks, stream);
  if (comm->nRanks == 1) {
    ...
    NCCLCHECK(saveKernel(ncclCollReduceScatter, sendbuff, recvbuff, count, datatype, op, root, comm, stream, nbytes*comm->nRanks, REDUCESCATTER_BUFCHUNKS, 1));
  }
  return ncclSuccess;
}
