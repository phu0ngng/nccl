...
static void *cudaLib;
static unsigned int cudaApiVersion;
...
#define LOAD_SYM(symbol) do {                                           \
    res = pfn_cuGetProcAddress(#symbol, (void **) (&pfn_##symbol), cudaDriverVersion, 0); \
    if (res != 0) {                                                     \
      WARN("Retrieve %s version %x failed with %d", #symbol, cudaDriverVersion, res); \
      return ncclSystemError;                                           \
    } } while(0)

  LOAD_SYM(cuGetErrorString);
  LOAD_SYM(cuGetErrorName);
  ...
  LOAD_SYM(cuMemExportToShareableHandle);
  LOAD_SYM(cuMemImportFromShareableHandle);
...
#if 0
  if (cudaApiVersion < CUDA_API_MIN_VERSION) {
    WARN("CUDA API version found is %d Minimum requirement is %d",
         cudaApiVersion, CUDA_API_MIN_VERSION);
    goto error;
  }
#endif
