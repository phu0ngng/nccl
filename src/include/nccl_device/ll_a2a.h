...
struct ncclSymLLA2ASession: ncclSymLLA2ASession_internal<Coop> {
  NCCL_DEVICE_INLINE ncclSymLLA2ASession(Coop, ncclDevComm const&, ncclTeam, ncclSymLLA2AHandle, uint32_t block, int maxElts, bool multimem=false, ncclMultimemHandle mmHandle={});

  NCCL_DEVICE_INLINE ncclSymLLA2ASession(Coop, ncclDevComm const&, ncclTeamTagLsa, uint32_t block, int maxElts, bool multimem=false);
