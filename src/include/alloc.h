...
static inline ncclResult_t ncclCuMemFree(void *ptr, struct ncclMemManager* manager = nullptr, int numSegments = 1) {
  if (ptr == NULL) return ncclSuccess;
  ncclResult_t result = ncclSuccess;
  size_t totalSize = 0;
  for (int segment = 0; segment < numSegments; segment++) {
...
static inline ncclResult_t ncclCuMemAlloc(void **ptr, void *handlep, int type, size_t size,
                                          struct ncclMemManager* manager = nullptr, ncclMemType_t memType = ncclMemScratch) {
...
ncclResult_t ncclCudaCallocDebug(T** ptr, size_t nelem, const char *filefunc, int line,
                                  struct ncclMemManager* manager = nullptr, ncclMemType_t memType = ncclMemPersist) {
  ncclResult_t result = ncclSuccess;
  cudaStreamCaptureMode mode = cudaStreamCaptureModeRelaxed;
  *ptr = nullptr;
  CUDACHECK(cudaThreadExchangeStreamCaptureMode(&mode));
...
template <typename T>
ncclResult_t ncclCudaFree(T* ptr, struct ncclMemManager* manager = nullptr, int numSegments = 1) {
  ncclResult_t result = ncclSuccess;
  cudaStreamCaptureMode mode = cudaStreamCaptureModeRelaxed;
