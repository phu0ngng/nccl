  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      ...
      for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
        ...
        if (a == NCCL_ALGO_TREE && p == NCCL_PROTO_LL128) busBw = std::min(busBw * (comm->nNodes == 1 ? 7.0/9.0 : ppn < 2 ? 0.85 : 120.0/128.0), ll128MaxBwPerCh[coll]*graphs[a]->nChannels*7.0/9.0);
