...
ncclResult_t ncclTopoTuneModel(struct ncclComm* comm, int minCompCap, int maxCompCap, struct ncclTopoGraph** graphs) {
  ...
  if (CLIQUE_NODES(comm) == 1) algoEnable[NCCL_ALGO_NVLS_TREE] = 0;

  // Disable CollNet if it is not supported
  if (comm->collNetSupport == 0) {
    algoEnable[NCCL_ALGO_COLLNET_DIRECT] = 0;
    algoEnable[NCCL_ALGO_COLLNET_CHAIN] = 0;
    if (CLIQUE_NODES(comm) > 1) algoEnable[NCCL_ALGO_NVLS] = 0;
...
ncclResult_t ncclTopoGetAlgoTime(struct ncclInfo* info, int algorithm, int protocol, int numPipeOps, float* time) {
  ...
  if (algorithm == NCCL_ALGO_RING && protocol == NCCL_PROTO_SIMPLE && CLIQUE_NODES(info->comm) > 1
